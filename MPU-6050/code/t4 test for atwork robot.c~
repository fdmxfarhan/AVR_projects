...
while (1)
    ...
    //delay_ms(300);
    x+=abs(ax); 
    glcd_putcharxy(40,0,((x/100)%10+'0'));
    glcd_putchar((x/10)%10+'0');
    glcd_putchar((x/1)%10+'0');
    
    y+=abs(ay); 
    glcd_putcharxy(40,10,((y/100)%10+'0'));
    glcd_putchar((y/10)%10+'0');
    glcd_putchar((y/1)%10+'0');
      */
    Get_Gyro_Val();
    a1=(Gyro_Raw_Val[Z]/1000);
    cmp+=a1;        
    if(cmp>255)  cmp=255;
    if(cmp<-255) cmp=-255;
    
    glcd_outtextxy(0,10,"CMP:");
    if(cmp>=0)
        {
        glcd_putchar('+');
        glcd_putchar((cmp/1000)%10+'0');
        glcd_putchar((cmp/100)%10+'0');
        glcd_putchar((cmp/10)%10+'0');
        glcd_putchar((cmp/1)%10+'0');
        }                       
    else 
        {
        glcd_putchar('-');
